...
void ScavTrap::guardGate(){
	...
	std::cout << GREEN "ScavTrap is now in Gate keeper mode." END << std::endl;
	this->isGuard = true;
}

void	ScavTrap::takeDamage( unsigned int amount ){

	std::cout << GREEN "--ScavTrap takeDamage function--" END << std::endl;
	if (this->isGuard == false)
	{
		ClapTrap::takeDamage( amount );
	}
	else
		std::cout << RED << this->name << " can't attack because he/she/it is keeper mode!" END << std::endl;
}
