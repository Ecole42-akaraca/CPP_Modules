...
/*
	"form.executive( *this );"'in başarılı bir şekilde çalıştığını şunu yazdırın demesinden dolayı try-catch yapısının kullanılması gerektiğini anlıyoruz.
*/
void	Bureaucrat::executiveForm( AForm const & form ){
	try
	{
		form.executive( *this );
		std::cout << BLUE << this->getName() << " executed " << form.getName() << END << std::endl;
	}
	catch(const std::exception& e)
	{
		std::cerr << PURPLE << this->getName() << " couldn't executed "\
