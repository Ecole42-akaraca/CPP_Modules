...
/*
	...
	-	std::cout << c << std::endl;
	-	std::cout << a << std::endl;
	Şeklindeki kullanımda a'nın değeri değişmez.
	Fixed&c = a; olarak c nesnesi oluşturulsaydı, a'nın değeri değişirdi.
*/
...
/*
(const Fixed &ref), ref.fixNumber = 999; şeklinde nesnenin argümanlarının değiştirilmesini önlüyor.

") const{" sağdaki const; soldaki nesnenin üzerinde değişiklik yapılmasını önlüyor.
örn: this->fixNumber = 9999; şeklinde kullanılmasını önlüyor.
*/
...
soldaki const değerinin anlamı:
	fonksiyonunun geri dönüş değerinin "const reference" olduğunu belirtir.
	"Const reference", bir referansın değerinin değiştirilemeyeceği anlamına gelir. 
*/
...
Fixed &Fixed::max( Fixed &lref, Fixed &rref ){
	...
	// Gelen argümanların referansları const tipinde gelmediğinden dolayı bulunduğu nesne yapısının değerleri değiştirilebilir.
	// örn: "lref.fixNumber = 9999999;" şeklinde değişiklik yapabiliriz. Bunu önlemek için argümanlar const tipinde çağırılır.
	if (lref.getRawBits() > rref.getRawBits())
		return (lref);
	return (rref);
}
